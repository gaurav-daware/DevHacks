...
from schemas.models import AIHintRequest, AIHintResponse
...
async def call_gemini(messages: list[dict], user_message: str) -> str:
...
async def get_hint(req: AIHintRequest):
    """
    ...
    context_parts = [
        f"Problem: {req.problem_description[:500]}",
        f"\nUser's {req.language} code:\n```{req.language}\n{req.code[:1000]}\n```",
    ]

    if req.error_output:
        context_parts.append(f"\nError output:\n{req.error_output[:300]}")
    ...
    try:
        hint = await call_gemini(req.conversation, user_message)
        return AIHintResponse(hint=hint)
    except Exception as e:
        return AIHintResponse(hint=generate_fallback_hint(str(e)))


@router.post("/analyze-session")
async def analyze_pair_session(request: dict):
    """
...
{{"analysis": "...", "strengths": "...", "improvement": "..."}}"""
    ...
    try:
        ...
        return {"analysis": "Session analyzed. Both contributors showed strong problem-solving skills."}
